...
iii <- (comadre$Genus == "Marmota") & (comadre$Species == "flaviventris")
comadre.marmota.flaviventris <- comadre[iii, ]

# Development note: save metadata for future use
# write.table(
#   cdb_metadata(comadre.marmota.flaviventris),
#   file = "C:/Users/Rich/projects/mpmaggregate/inst/extdata/marmota-flaviventris.csv",
#   sep = ",",
#   col.names = NA
# )

# Return the matrices associated with MatrixID `matid`
get_matrix <- function(matid) {
  comadre2 <- comadre.marmota.flaviventris[comadre.marmota.flaviventris$MatrixID == matid, ]
  matA <- matA(comadre2)[[1]]
  matU <- matU(comadre2)[[1]]
  matF <- matF(comadre2)[[1]]
  matC <- matC(comadre2)[[1]]
  list(matA = matA, matU = matU, matF = matF, matC = matC)
}

mats <- get_matrix(249455)

# Extract matrices and fold any C component into F
matA <- mats$matA
matU <- mats$matU
matF <- mats$matF
matC <- mats$matC
...
E_A <- elasticity(matA = matAk, framework = "lambda")$elasticity

E_list <- list(
  "Original"            = P %*% E_A %*% t(P),
  "Agg λ + standard"    = elasticity(matA = m1$A, framework = "lambda")$elasticity,
  "Agg λ + elasticity"  = elasticity(matA = m2$A, framework = "lambda")$elasticity,
...
E_A <- elasticity(matF = Rk, matU = Uk, framework = "R0", normalize = TRUE)$elasticity

E_list2 <- list(
  "Original"            = P %*% E_A %*% t(P),
  "Agg λ + standard"    = elasticity(matF = m1$F, matU = m1$U, framework = "R0", normalize = TRUE)$elasticity,
  "Agg λ + elasticity"  = elasticity(matF = m2$F, matU = m2$U, framework = "R0", normalize = TRUE)$elasticity,
...
of stages. Theoretical Population Biology, 115, 69-80. https://doi.org/10.1016/j.tpb.2017.04.002
...
Ozgul, A., Oli, M.K., Armitage, K.B., Blumstein, D.T. and Van Vuren, D.H., 2009.
Influence of local demography on asymptotic and transient dynamics of a yellow-bellied
marmot metapopulation. The American Naturalist, 173(4), 517-530.
